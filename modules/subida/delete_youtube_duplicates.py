...
# Asegurar que el directorio de este script esté en sys.path para importaciones relativas
...
def main():
    ...
    for (manga_id, title), group in videos_by_title.items():
        ...
        public_videos = [v for v in group if v["privacyStatus"] == "public"]
        
        # Selección:
        # 1. Si hay públicos, preferir el público con más views (o el más antiguo ante empates).
        # 2. Si son todos programados/privados, preferir el programado para más temprano.
        if public_videos:
            def sort_public(v):
                try:
            ...
            public_sorted = sorted(public_videos, key=sort_public)
            chosen = public_sorted[0]
        else:
            ...
            scheduled_sorted = sorted(group, key=sort_scheduled)
            chosen = scheduled_sorted[0]
        ...
        for v in group:
            if v["id"] != chosen["id"]:
                ...
                print(f"  [ELIMINAR] ID: {v['id']} | Estado: {v['privacyStatus']} | Vistas: {v['views']} | {sched_info}")
                
        chosen_sched = f"Programado: {chosen['publishAt']}" if chosen['publishAt'] else f"Subido: {chosen['publishedAt']}"
        print(f"  [CONSERVAR] ID: {chosen['id']} | Estado: {chosen['privacyStatus']} | Vistas: {chosen['views']} | {chosen_sched}")
...
def mock_run():
    ...
    mock_data = [
        ...
        {"id": "v2", "title": "Manga Recap 1", "description": "Un gran recap #manga1 #shorts", "privacyStatus": "private", "publishAt": "2026-06-01T15:00:00Z", "publishedAt": "2026-06-01T10:05:00Z", "views": 0},
        ...
        {"id": "v4", "title": "Manga Recap 2", "description": "Otro recap #manga2 #shorts", "privacyStatus": "private", "publishAt": "2026-06-02T15:00:00Z", "publishedAt": "2026-06-01T11:05:00Z", "views": 0},
    ...
    for (manga_id, title), group in videos_by_title.items():
        ...
        if public_videos:
            chosen = sorted(public_videos, key=lambda v: (-v["views"], v["publishedAt"]))[0]
        else:
            chosen = sorted(group, key=lambda v: (v["publishAt"] or v["publishedAt"]))[0]
